...
class UserInfo:
    ...
    shop_id: str    # айди ммагазина на Lava
    ...
    def __init__(self, fields: Tuple = None, token: str = None, shop_id = None, name: str = None,
        ...
        :param shop_id: Айди проетка на Lava. Перезаписывает айди из fields.
        ...
        if len(fields) == 6:
            self.token, self.shop_id, self.name, self.percent, self.withdraw_service, self.withdraw_wallet = fields
...
def __create_table(connection: Connection, cursor: Cursor):
    cursor.execute(
        ...
        "(token VARCHAR(32) NOT NULL, shop_id VARCHAR(32) NOT NULL, name VARCHAR(32) NOT NULL, percent INT UNSIGNED NOT NULL, "
...
def __add_user(connection: Connection, cursor: Cursor, name: str, shop_id: str, percent: int, withdraw_service: str, withdraw_wallet: str, token: str = None):
    if token is None:
        token = secrets.token_urlsafe(32)    # генерирует токен из случайных символов длиной 32 символа

    cursor.execute("INSERT INTO users (token, shop_id, name, percent, withdraw_service, withdraw_wallet) VALUES (?, ?, ?, ?, ?, ?);",
                   (token, name, shop_id, percent, withdraw_service, withdraw_wallet))
    connection.commit()
...
if __name__ == "__main__":
    ...
    #print(__add_user(dbm.connection, dbm.cursor, "123-shop-id-321", "SOZ", 100, "qiwi", "+79608357711"))
    dbm.get_user_info("0WH5bBJrq2nA1x6Wpk85vxBvOpC5zLydSuj9YIcOPa8")
